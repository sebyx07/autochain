...
const Car =  ApplicationRecord.extend({
  ...
  attrs: {
    'model': attr(),
    'brand': attr(),
    'first-registration': attr(),
    'color': attr(),
    'number-of-kilometers': attr(),
    'price-euro': attr(),
    'number-of-doors': attr(),
    'transmission-type': attr(),
    'description': attr(),
    'images': attr()
  }
})
...
export class SearchComponent implements OnInit {
  ...
  @Output('carsEvent')
  carsEventEmitter: EventEmitter<any> = new EventEmitter<any>()
  ...
  search() {
    ...
    Car
      .where({
        ...
        'price_euro': (this.priceValues[0] * 600 + 1000).toString()  + ',' + (this.priceValues[1] * 600 + 1000).toString(),
        'fuel_type': this.selectedType,
        'first_registration': (this.yearValues[0] / 5 + 1998).toString() + ',' + (this.yearValues[1] / 5 + 1998).toString()
      })
      .all().then( response => {
        this.carzz = response.data;
    });
  }
